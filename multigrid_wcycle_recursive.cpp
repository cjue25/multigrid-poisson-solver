...
void multigrid_cycle(double ***phi, double ***f, double ***aux, int n_nodes,
                     int n_sweeps, int n_levels, int flevel, int mg_return);
...
int main(int argc, char* argv[]) {
  ...
  int i_mgcycles = 0, n_nodes = NUM_NODES, mg_levels = 0, mg_return = 4;
  ...
  for (i_mgcycles = 1; i_mgcycles <= n_mgcycles; i_mgcycles++) {
    ...
    multigrid_cycle(phi, f, aux, n_nodes, n_sweeps, n_levels, FINE_MESH, mg_return);
...
void multigrid_cycle(double ***phi, double ***f, double ***aux, int n_nodes,
                     int n_sweeps, int n_levels, int flevel, int mg_return) {
    int level = mg_return;
    if (flevel < n_levels ) level = flevel;
    if (flevel > n_levels-1 and 2*n_levels -mg_return-2 >flevel) level = 2*(n_levels -1)-flevel ;
//  printf("%i\n" , level);
    smooth_total(phi,f,aux,n_nodes,n_sweeps,level);

    if (flevel < n_levels-1) {
        restrict_weighted(phi, f, aux, n_nodes, level);
        int n_coarse     = (n_nodes-1)/(2) + 1;
        int level_coarse = flevel + 1;
        multigrid_cycle(phi, f, aux, n_coarse, n_sweeps, n_levels, level_coarse, mg_return);
        prolongate_weighted(phi, aux, n_nodes, level);
    }
    if(flevel > n_levels - 2 and 2*n_levels - mg_return-2 > flevel){
        int plevel        = level - 1;
