...
void smooth_sor(double **phi, double **f, double **aux, int n_nodes,
  ...
  double h2 = pow(1.0/((double)n_nodes-1.0),2.0);
   ...
   const int SendCount = n_nodes*n_nodes/2;
   const int RecvCount = SendCount;
   
   const int tag = 0;
   float  RecvBuf[n_nodes/2][n_nodes];
     float r1_bc[n_nodes];
   float bc_rev[n_nodes];

   const int TargetRank = (MyRank+1)%2;
  for (int iter = 0; iter < n_sweeps; iter++) {
  
  MPI_Scatter( &phi, SendCount, MPI_FLOAT,&RecvBuf, RecvCount, MPI_FLOAT,RootRank, MPI_COMM_WORLD );
  if (MyRank == RootRank){
   ...
   if (MyRank == RootRank){
       ...
       MPI_Recv(&bc_rev,n_nodes,MPI_FLOAT,TargetRank,tag,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
       const int t=n_nodes/2-1;
       for (int j=1;j<(n_nodes-1);j++){
   ...
   MPI_Ssend(&r1_bc,n_nodes,MPI_FLOAT,TargetRank,tag,MPI_COMM_WORLD);
   }
   else {
      for (int i=0;i<n_nodes;i++){
       r1_bc[i]=RecvBuf[0][i];
       MPI_Send(&r1_bc,n_nodes,MPI_FLOAT,TargetRank,tag,MPI_COMM_WORLD);
       }
       MPI_Recv(&bc_rev,n_nodes,MPI_FLOAT,TargetRank,tag,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
       for (int j=1;j<(n_nodes-1);j++){
...
// Gather 
  ...
  MPI_Gather( &RecvBuf, SendCount, MPI_FLOAT, &phi, RecvCount,MPI_FLOAT,
