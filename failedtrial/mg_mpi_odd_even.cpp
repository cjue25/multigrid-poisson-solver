...
void smooth_sor(double **phi, double **f, double **aux, int n_nodes,
  ...
  int tag = n_nodes;
  int done_tag=n_nodes*3;

  int eventag=n_nodes*5;
  int even_done_tag=n_nodes*7;
  
  int phitag=n_nodes*9;
  int n2=(n_nodes-1)/2;
 
  int mv_node;
  double relax = 1.7;
  double h2 = pow(1.0/((double)n_nodes-1.0),2.0);
  double SendBuf[n2+1][n_nodes];
  double RecvBuf[n2+1][n_nodes];
  double NewSendBuf[n2+1][n_nodes];
  
  double evenSendBuf[n2+1][n_nodes];
  double evenRecv[n2+1][n_nodes];
  double evenNew[n2+1][n_nodes];
  
  double sendfinal[n_nodes][n_nodes]; 
  double final[n_nodes][n_nodes];
  
  double count=(n2+1)*n_nodes;
...
for (int iter = 0; iter < n_sweeps; iter++) {
  ...
  MPI_Recv(&NewSendBuf,count,MPI_DOUBLE,TargetRank,done_tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
  for (int i=1;i<n2;i++){
      for (int j=0;j<n_nodes;j++){
        phi[i+n2][j]=NewSendBuf[i][j];
      }//i
    }//get odd total j
  ...
  //give sendbuf
	for (int i=0;i<n2+1;i++){
		for (int j=0;j<n_nodes;j++){
		evenSendBuf[i][j]=phi[i+n2][j];
		}//j
	}//i

	MPI_Send(&evenSendBuf,count,MPI_DOUBLE,TargetRank,eventag,MPI_COMM_WORLD);
  	for (int i = 1;i<n2+1;i++){
    if (i%2!=0){mv_node=2;}
  ...
  MPI_Recv(&evenNew,count,MPI_DOUBLE,TargetRank,even_done_tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
  for (int i=1;i<n2;i++){
      for (int j=0;j<n_nodes;j++){
        phi[i+n2][j]=evenNew[i][j];
  ...
  for (int i=0;i<n_nodes;i++){
    for (int j=0;j<n_nodes;j++){
    sendfinal[i][j]=phi[i][j];
    printf("%f ",phi[i][j]);
    }
    printf("\n");
    }
    
  MPI_Ssend(&sendfinal,n_nodes*n_nodes,MPI_DOUBLE,TargetRank,phitag,MPI_COMM_WORLD);
  }//root rank
  ...
  MPI_Recv(&RecvBuf,count,MPI_DOUBLE,TargetRank,tag,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    for (int j=1;j<(n_nodes-1);j+=2){
          RecvBuf[i][j] = (1.0 - relax)*RecvBuf[i][j] + relax*(RecvBuf[i][j-1] + RecvBuf[i-1][j] +\
                           RecvBuf[i+1][j] + RecvBuf[i][j+1] + h2*f[i+n2][j])/4.0;
    }//for j
    }//for i
    MPI_Send(&RecvBuf,count,MPI_DOUBLE,TargetRank,done_tag,MPI_COMM_WORLD); 
    //printf("Send RecvBuf");
    // even
    MPI_Recv(&evenRecv,count,MPI_DOUBLE,TargetRank,eventag,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    //printf("Recv evenRecv");
    for (int i=1;i<n2;i++){
    if (i%2!=0){mv_node=2;}
    else if (i%2==0){mv_node=1;}
    for (int j=1;j<(n_nodes-1);j+=2){
          evenRecv[i][j] = (1.0 - relax)*evenRecv[i][j] + relax*(evenRecv[i][j-1] + evenRecv[i-1][j] +\
                           evenRecv[i+1][j] + evenRecv[i][j+1] + h2*f[i+n2][j])/4.0;
    
    
    }//for j
    }//for i
    MPI_Send(&evenRecv,count,MPI_DOUBLE,TargetRank,even_done_tag,MPI_COMM_WORLD); 
    MPI_Recv(&final, n_nodes*n_nodes, MPI_DOUBLE, TargetRank, phitag, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
    //printf("Rank %d\n",MyRank);
    for (int i=0;i<n_nodes;i++){
    for (int j=0;j<n_nodes;j++){
    phi[i][j]=final[i][j];
...
double compute_residual(double **phi, double **f, double **residual,
  ...
  for (int i = 1; i < n_nodes-1; i++) {
    for (int j = 1; j < n_nodes-1; j++) {
      ...
      //                            phi[i+1][j] + phi[i][j+1] - 4.0*phi[i][j]));                          
                                  
      norm += residual[i][j]*residual[i][j];
    }
  }
  norm = sqrt(norm);
  //printf("norm %f rank %d",norm, MyRank);
